...
export class PlanetasService {
  ...
  obtenerMenu = () =>
    this.http.get('https://swapi.co/api/planets/').pipe(
      expand((res: SResponse<Planetas>) => {
      ...
      map((res: SResponse<Planetas>) => res.results.map(toMenuItem)),
      reduce((anterior, actual) => [...anterior, ...actual]) // Junta todos los items en una sola lista
    )

  obtener = (id: number | string) =>
    this.http.get<Planetas>(`https://swapi.co/api/planets/${id}/`)

  buscar = (params: HttpParams) =>
    this.http
      .get<SResponse<Planetas>>(`https://swapi.co/api/planets/`, {
        params
      })
      .pipe(
        map(res => {
