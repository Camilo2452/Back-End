...
export class EspeciesService {
  ...
  obtenerMenu = () =>
    this.http.get('https://swapi.co/api/species/').pipe(
      expand((res: SResponse<Especies>) => {
      ...
      map((res: SResponse<Especies>) => res.results.map(toMenuItem)),
      reduce((anterior, actual) => [...anterior, ...actual]) // Junta todos los items en una sola lista
    )

  obtener = (id: number | string) =>
    this.http.get<Especies>(`https://swapi.co/api/species/${id}/`)

  buscar = (params: HttpParams) =>
    this.http
      .get<SResponse<Especies>>(`https://swapi.co/api/species/`, {
        params
      })
      .pipe(
        map(res => {
